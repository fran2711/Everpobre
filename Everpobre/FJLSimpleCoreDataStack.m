...
@interface FJLSimpleCoreDataStack ()
...
@implementation FJLSimpleCoreDataStack
...
- (NSPersistentStoreCoordinator *)storeCoordinator
{
    if (_storeCoordinator == nil) {
        ...
        //Opciones de migración de datos automáticas de forma light
        ...
        if (![_storeCoordinator addPersistentStoreWithType: NSSQLiteStoreType
            ...
            NSNotification *note = [NSNotification
                                    notificationWithName:[FJLSimpleCoreDataStack persistentStoreCoordinatorErrorNotificationName]
                                    object:self
                                    userInfo:@{@"error" : err}];
...
+ (FJLSimpleCoreDataStack *)coreDataStackWithModelName:(NSString *)aModelName
...
{
    return [[self alloc] initWithModelName:aModelName
                               databaseURL:aDBURL];
}
...
- (void)saveWithErrorBlock:(void(^)(NSError *error))errorBlock
{
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"FJLSimpleCoreDataStack"
                                  code:1
